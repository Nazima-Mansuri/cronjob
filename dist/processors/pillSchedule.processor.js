...
function onPillScheduleProcess(job, done) {
    ...
    getPillData(schema, pillID).then(onSuccess).catch(function () {
    ...
    pool.connect(function (err, client, done) {
        if (err) {
            ...
            userIds.forEach(function (item) {
                client.query('INSERT INTO ' + schema + '.feeddelivery (territory, userid, feed, pillid,deliverytime,createdate) VALUES ($1, $2, $3, $4, $5,$6) RETURNING *;', [territory, item, feedId, pillId, now, now], function (err, result) {
                    if (err) {
...
function addFailureMessage(obj) {
    ...
    return new Promise(function (callback, reject) {
        ...
        pool.connect(function (err, client, done) {
            ...
            client.query('INSERT INTO ' + schema + '.feeddeliveryfail (territory, userid, feed, pillid,deliverytime,createdate) VALUES ($1, $2, $3, $4, $5,$6);', [territory, userId, feedId, pillId, now, now], function (err, result) {
                done();
                if (err) {
